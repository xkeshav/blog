...
export const getShareUrl = (platform: string, astroUrl: URL, description = "") => {
  const url = astroUrl as unknown as string;
  let shareUrl = "";
  ...
  switch (platform) {
    ...
    case "twitter":
      ...
      break;
    case "linkedin":
      shareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}&summary=${encodeURIComponent(
        description,
      )}`;
      break;
    case "pinterest":
      shareUrl = `https://www.pinterest.com/pin/create/button/?url=${encodeURIComponent(url)}&description=${encodeURIComponent(
        description,
      )}`;
      break;
    case "whatsapp":
