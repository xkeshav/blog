...
- create an account on [npmjs website](https://npmjs.com), ite free.
...
2. create new folder ( in my case, its _npm/demo_ ) and navigate it from terminal and run

```sh title="npm/demo"
> npm init
```

3. this will ask few questions and when finished , it will generate a _package.json_ file in your folder,
...
here important properties are _files_, _name_ and _main_

- _files_ will contain files, which will be downloaded when someone do `npm install your package`
- _name_ is the name of package which is used to indicate when someone do npm install, in my case here it will be `npm install @xkeshav/day`
- we are using typescript so _main_ and _types_ will be there and there file location will be from _lib_ folder

4. create _tsconfig.json_ file

```sh
> tsc -- init
```

it will generate default tsconfig.json, you can remove many of rules. below is my _tsconfig.json_ file content
...
Note: here `outDir` will be same as `files` in _package.json_

5. now create _src/index.ts_ file and add your own code which you want to include in the package, use `export` keyword; below is the content of file
...
4. build the package before publish and check the code in _lib_ folder
...
above command will add entry of the package in the _package.json_ file as below
...
alternatively you can add this command in _package.json_ script ( shown in earlier code snippet )and run `npm run publish`

## Update the version of package

if you want to update your package then

- either update the _version_ value in the _package.json_
